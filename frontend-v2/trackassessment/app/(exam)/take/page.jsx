            ...
            {result ? (
                <div className="bg-stone-600/10 rounded-xl p-8 mb-8">
                    ...
                    <div className="text-xl mb-4">
                        Score: <span className="font-bold text-purple-500">{result.score}%</span>
                    </div>
                    ...
                    <div className="space-y-6">
                        {questions.map((question) => {
                            ...
                            return (
                                <div key={question.number} className={`p-4 rounded-lg ${isCorrect ? 'bg-green-900/20 border border-green-500/30' : 'bg-red-900/20 border border-red-500/30'}`}>
                                    ...
                                    {!isCorrect && (
                                        <p className="text-sm">
                                            Correct answer: <span className="text-green-400">{question.Answers}</span>
                                        </p>
                                    )}
                                </div>
                            );
                ...
                <form onSubmit={handleSubmit} className="space-y-8">
                    {questions.map((question) => (
                        <div key={question.number} className="bg-stone-600/10 rounded-xl p-6">
                                ...
                                {['A', 'B', 'C', 'D'].map((option) => (
                                    ...
                                    >
                                        ...
                                        <span className="flex-1">
                                            <span className="font-semibold">{option}.</span> {question[option]}
                                        </span>
                                    </label>
                                ))}
                            </div>
                    ...
                    {Object.keys(answers).length < questions.length && (
                        <p className="text-center text-sm text-yellow-400">
                            Please answer all questions before submitting ({Object.keys(answers).length}/{questions.length} answered)
                        </p>
                    )}
                </form>
            )}
