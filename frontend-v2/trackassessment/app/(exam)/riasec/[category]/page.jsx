...
export default function RiasecCategoryPage() {
  ...
  // ✅ constants kept in one place
  const ASSESSMENT = "riasec";
  const BASE_PATH = "/riasec";
  const NEXT_MODULE_START = "/msa/VA"; // ✅ riasec -> msa
  ...
  // ✅ helper: redirect to next unanswered category (or to MSA if done)
  ...
  useEffect(() => {
    async function init() {
      ...
      // 2) categories for this assessment only
      ...
      const catList = cats || [];
      if (!catList.length) {
        setErrorMsg("No RIASEC categories configured.");
        setLoading(false);
        return;
      }
      ...
      // 3) resolve current category by CODE
      ...
      // 4) validation: if already answered -> go next unanswered
  ...
  // ✅ helper: go to the next category in sequence (or MSA if finished)
  const goNext = () => {
